...
Mirror the risk specialist's test surface — the two share the
:mod:`alphamind.agents.specialists._base` machinery but each gets its
own coverage rather than relying on transitivity.
"""
...
def _state() -> ResearchState:
    return ResearchState(
        query="How is NVDA management framing the China outlook?",
        as_of=date(2024, 12, 31),
        top_k=3,
    )


async def test_emits_findings_tagged_with_sentiment(sample_sources: list[Source]) -> None:
    payload = json.dumps(
        {
            "findings": [
                {
                    "claim": (
                        "Management uses 'cannot assure' framing when describing "
                        "future China sales."
                    ),
                    "cited_chunk_ids": [103],
                },
                {
                    "claim": "Data Center commentary leads the gross-margin narrative.",
                    "cited_chunk_ids": [102],
                },
            ]
        }
...
async def test_drops_findings_with_invalid_cites(
    ...
    payload = json.dumps({"findings": [{"claim": "fabricated", "cited_chunk_ids": [9999]}]})
    client = ScriptedLLMClient(responses=[payload])
