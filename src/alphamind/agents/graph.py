...
intent the router produced. LangGraph fans in at the synthesizer —
all selected specialists must complete before synthesis runs.

``fundamentals`` is always included in the fan-out as a baseline, even
if the router omits it. The other specialists are gated by intent.

The technical specialist is a placeholder — it accepts the same call
signature as the others and short-circuits with no findings until the
market-data adapter exists. See
:mod:`alphamind.agents.specialists.technical` for the rationale.
...
_WIRED_SPECIALISTS: frozenset[str] = frozenset({"fundamentals", "sentiment", "risk", "technical"})

# Re-export so callers can import a single name for the retrieval contract.
__all__ = ["RetrievalFn", "build_research_graph"]


_RoutedSpecialist = Literal["fundamentals", "sentiment", "risk", "technical"]
...
def build_research_graph(
    *,
    ...
    # signature under strict mypy. Suppression is scoped to these lines;
    # nothing else in the agent layer needs the escape hatch.
