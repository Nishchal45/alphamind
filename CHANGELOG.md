...
### Added
- `SentimentSpecialist` (`alphamind.agents.specialists.sentiment`) — qualitative-tone analyst with a new `SENTIMENT_SYSTEM` prompt. Shares the `make_specialist_node` machinery with fundamentals / risk; only the bound prompt and the emitted-finding's `specialist` tag differ. System prompt is explicit that the strongest sentiment signal (earnings transcripts) is not yet ingested.
- `TechnicalSpecialist` (`alphamind.agents.specialists.technical`) — registered no-data placeholder. Same `(client, retrieve, *, model, max_tokens)` factory signature as the other specialists so the graph builder doesn't need a special case; the node short-circuits before retrieval / LLM and returns `{"sources": [], "findings": [], "usage": []}` with the documented reason. Drops the override and falls through to `make_specialist_node` once the market-data adapter exists.
- `TECHNICAL_SYSTEM` prompt kept alongside the others for the day the adapter lands; documented as inactive while the stub is in place.
- Graph wiring: `_WIRED_SPECIALISTS` and the conditional-edges path map now cover all four specialists. Fan-in at the synthesizer is unchanged — selected specialists fan out, the synthesizer waits.
- Tests: per-specialist test files for sentiment and technical, plus an end-to-end graph test that fans out across fundamentals + sentiment + technical and asserts the technical stub doesn't show up in usage or findings.
