...
### Added
- `alphamind.api` package: FastAPI app via `create_app()` factory + lifespan. Exposes `POST /research` (Server-Sent Events stream of agent-graph progress), `GET /healthz` (liveness, no I/O), `GET /readyz` (verifies Postgres reachability). `make serve` runs `uvicorn --factory` against it.
- SSE adapter (`alphamind.api.sse`) converts LangGraph `astream(stream_mode="updates")` payloads to typed Server-Sent Events: `router-intent`, `specialist-findings` (one per specialist node, in completion order, grouped by specialist), `thesis`, `critique`, then `done` (or `error` on graph failure). Token-level streaming of the synthesizer answer is **not** in this cut — it requires growing the `LLMClient` protocol and lands in a follow-up.
- Pydantic schemas (`alphamind.api.schemas`) mirror the agent dataclasses (`Source`, `Finding`, `RouterIntent`, `Thesis`, `ThesisClaim`, `CritiqueIssue`, `Critique`, `Usage`) at the HTTP boundary. Separate by design — different audiences (internal transport vs OpenAPI surface) — with a one-line `from_dataclass` per record. `ResearchRequest` validates `query` (non-blank), `as_of` (mandatory, no default), `top_k` ([1, 50]).
- `alphamind.api.retrieval.build_hybrid_retrieval`: wires `HybridSearch` into the agent layer's `RetrievalFn` contract for the API's lifespan. Each retrieve call opens its own DB session so parallel specialists don't serialise on a shared session.
- `fastapi`, `uvicorn[standard]`, and `sse-starlette` added to `dependencies`.
- ADR 0009 documenting FastAPI + SSE choices: why SSE over WebSockets, factory + lifespan over a module-level app, pydantic schemas separate from agent dataclasses, per-specialist event grouping over flat accumulator streams, what's deferred (token streaming, Redis cache, cost routing, auth).
