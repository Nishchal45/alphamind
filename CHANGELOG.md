...
### Added
- `alphamind.backtest` package: historical SPY backtest harness for the research DAG. Walks a YAML universe of `(case_id, ticker, query, as_of)` cases, runs the production graph at each historical as-of, extracts a mechanical signal from the thesis (`(n_bull - n_bear) / (n_bull + n_bear)` thresholded into long / short / flat), and compares forward returns to SPY over the same window. Per-case failures (graph errors, missing prices) are isolated to the row and surfaced in the report.
- Mechanical signal extraction (`extract_signal`): deterministic, no extra LLM cost, no extra leakage surface vs. an LLM-rated alternative. Empty / None theses resolve to flat. ADR 0010 documents the trade-off vs. a weighted variant.
- Price adapter (`YahooPriceSource`): yfinance with a CSV disk cache under `data/prices/` (gitignored). First run hits Yahoo, subsequent runs are pure disk reads. The `PriceSource` Protocol means tests inject stub price series and never touch the network. Helper functions `entry_price` / `exit_price` enforce the time-horizon defense from ADR 0010 — entry is the first trading day **strictly after** `as_of` (the as-of-day price is never used), exit is the last trading day on or before `as_of + horizon_days`.
- Portfolio simulator + metrics (`alphamind.backtest.metrics`): position-aware return (sign-flip for shorts), cumulative-product equity curve ordered by `as_of`, hit rate, mean alpha vs benchmark, descriptive CAGR / Sharpe annualised over the active span, max drawdown. Pure functions over `Sequence[CaseResult]`; flat and errored cases are excluded from active-only aggregates.
- YAML universe loader (`load_universe`): strict validation up front — missing fields, bad dates, duplicate case ids, non-positive horizons, non-numeric thresholds all raise `UniverseError` at load time. Mirrors the eval golden-set loader pattern.
- Report writers: markdown (`write_markdown`) opens with the methodological caveats per ADR 0010 — not buried at the end — then run config, aggregate metrics, equity-curve PNG link, per-case table. JSON sidecar (`write_json`) for machine consumers. Equity-curve chart (`write_chart`) via matplotlib, headless. The chart writer no-ops cleanly when no case took a position.
- `scripts/backtest.py` + `make backtest`: CLI wires the production graph (`HybridSearch` + LLM factory) through the runner. Outputs go to `docs/eval/backtest.md` (human-facing), `evals/backtest_report.json` (machine-readable), `docs/eval/backtest_equity.png` (chart). Exit code is non-zero only when every case errored.
- `evals/backtest_universe.yaml`: seed universe with four (ticker, as_of) cases covering NVDA, AAPL, MSFT across 2023 Q3 — 2024 Q1.
- `yfinance>=0.2.40`, `matplotlib>=3.9`, `pandas>=2.2` added to `dependencies`. Mypy override added for `yfinance.*`, `matplotlib.*`, `pandas.*` (they ship without `py.typed`).
- ADR 0010 documenting the backtest design: signal-extraction choice, three-layer lookahead defense (agent DAG → entry-price strict-after rule → exit-price horizon target), why this is a sanity check and not a tradable-alpha claim, what's deliberately not measured (transaction costs, slippage, survivorship, risk-adjusted sizing, out-of-sample split, cross-sectional ranking).
- Runbook `docs/runbooks/backtest.md` with the CLI, the failure-mode catalogue, and the same "this isn't financial advice" framing the report itself opens with.
- Tests under `tests/unit/backtest/` (54 cases) covering: signal thresholding edge cases, entry/exit price selection across weekend gaps, hit-rate / mean-alpha / drawdown / CAGR / Sharpe under small samples, YAML loader error paths, end-to-end runner happy path (long + short), per-case error isolation (graph failure, missing prices), and report writers (markdown, JSON, chart).
