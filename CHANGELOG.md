...
### Added
- `alphamind.training` package: the full QLoRA fine-tune pipeline for a per-chunk claim-extraction SLM (ADR 0011). The fine-tune target is deliberately narrower than a specialist — given one filing chunk, emit the material claims as JSON — so it's learnable on a 1.5B model and the cross-chunk citation logic stays in the (deterministic) orchestration layer.
- Distillation dataset builder (`build_examples`): runs filing chunks through the frontier model (the `LLMClient` Protocol, so it's testable against a stub — no API call in CI) and captures `{"claims": [...]}` targets. Teacher outputs that don't parse are dropped and counted. JSONL read/write and a deterministic train/val split (hashed by chunk id, so adding chunks doesn't reshuffle the existing partition).
- QLoRA trainer (`alphamind.training.trainer`): `TrainConfig` with ADR-0011 defaults (Qwen2.5-1.5B-Instruct base, 4-bit NF4, rank 16 / alpha 32, attention + MLP target modules). The config assembly (`build_lora_kwargs`, `build_quant_kwargs`) and chat rendering (`example_to_chat`) are pure and unit-tested; the `train()` loop lazily imports torch / peft / trl / transformers and raises a clean `TrainingDependencyError` when the `train` extra is absent.
- Claim-extraction evaluator (`alphamind.training.evaluate`): greedy one-to-one claim matching on token-overlap (Jaccard ≥ threshold), micro-averaged precision / recall / F1, and a valid-JSON rate. `evaluate_model` runs predictions through any `LLMClient`, so the same code scores the base model, the fine-tuned adapter, or the frontier teacher.
- `alphamind.llm.local.LocalLLMClient`: vLLM-backed `LLMClient` implementation (ADR 0012) serving a base model + optional LoRA adapter. vLLM is imported lazily (the `serve-local` extra); the message-rendering logic is pure and tested, the engine build + generation are isolated behind the lazy import and dispatched to a worker thread. `llm_backend='local'`, `slm_base_model`, `slm_adapter_path` settings + factory branch.
- `scripts/build_training_set.py`, `scripts/train_slm.py`, `scripts/eval_slm.py` + `make train-data` / `train-slm` / `eval-slm`. The repo ships the pipeline, not a trained checkpoint — running the fine-tune is a documented GPU step.
- `train` extra (torch, transformers, peft, trl, datasets, accelerate, bitsandbytes) and `serve-local` extra (vllm), both optional and GPU-oriented. Mypy overrides for the stub-less heavy modules.
- ADR 0011 (fine-tune design: task framing, base-model choice, distillation, QLoRA config, eval metric, the GPU-is-manual boundary) and ADR 0012 (local vLLM backend: engine choice, Protocol fit, config, deferred items).
- Runbook `docs/runbooks/fine-tune.md`: the end-to-end distil → train → eval procedure, hyperparameter table, and failure-mode catalogue.
