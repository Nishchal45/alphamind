...
### Added
- Sentiment and technical specialists wired into the research DAG, completing the four-specialist topology described in ADR 0007. Each is a thin wrapper around `make_specialist_node` mirroring the fundamentals / risk pattern; the lift is in the prompts. `SENTIMENT_SYSTEM` targets the Loughran-McDonald / Tetlock filing-tone tradition (hedging, forward-looking language, defensive vs. confident phrasing); `TECHNICAL_SYSTEM` targets quantitative trend signals visible in filings (direction and rate-of-change in operating data). Both share the existing filing-chunks retrieval surface today — their natural sources (earnings-call transcripts for sentiment, market data for technical) remain unbuilt and are noted as deferred in both module docstrings and the prompts themselves so the constraint is honest rather than disguised.
- Graph topology: `_route_specialists` now dispatches up to four specialist nodes in parallel and the synthesizer fans in on all selected. `fundamentals` stays the forced baseline; the other three are gated by the router's intent so a narrowly fundamentals-shaped question doesn't pay for four LLM calls when one is enough.
- Tests: `test_sentiment.py`, `test_technical.py` mirror the existing per-specialist coverage (valid-cites pass-through, invalid-cite drop, no-sources short-circuit, parse-failure handling). `test_dag_fans_out_to_all_four_specialists` exercises the 4-way fan-out at the graph level via `SystemKeyedLLMClient`.
